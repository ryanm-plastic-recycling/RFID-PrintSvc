...
test("template lab page, preview, and test send stay outside production queue", async () => {
  ...
  assert.equal(result.text.includes("Pin Preview"), true);
  assert.match(result.text, /<input id="pinPreviewToggle" type="checkbox" checked>/);
  assert.equal(result.text.includes("Show Render Details"), true);
  assert.match(result.text, /<input id="showRenderDetailsToggle" type="checkbox">/);
  assert.equal(result.text.includes("id=\"renderDetailsPanel\" class=\"render-details-panel hidden\""), true);
  assert.equal(result.text.includes("id=\"compactRenderStateBadge\""), true);
  assert.equal(result.text.includes("Sticky Preview"), true);
  assert.equal(result.text.includes("Send Proof Print"), true);
  assert.equal(result.text.includes("Confirm proof print"), true);
  assert.equal(result.text.includes("id=\"templateLabTopbar\" class=\"template-lab-sticky-zone template-lab-topbar\""), true);
  assert.equal(result.text.includes("id=\"templateLabBody\" class=\"template-lab-body preview-pinned\""), true);
  assert.equal(result.text.includes("id=\"templateLabControlsScroll\" class=\"template-lab-controls-scroll\""), true);
  assert.equal(result.text.includes("id=\"templateLabWorkbench\" class=\"template-lab-workbench\""), true);
  assert.equal(result.text.includes("id=\"stickyPreviewRail\" class=\"template-lab-preview-rail\""), true);
  assert.ok(result.text.indexOf("template-lab-filter-bar") > result.text.indexOf("id=\"templateLabTopbar\""));
  assert.ok(result.text.indexOf("template-lab-action-bar") > result.text.indexOf("id=\"templateLabTopbar\""));
  assert.ok(result.text.indexOf("id=\"stickyPreviewRail\"") > result.text.indexOf("id=\"templateLabControlsScroll\""));
  ...
  assert.equal(result.status, 200);
  assert.equal(result.text.includes("Template Quick Edit"), true);
  assert.equal(result.text.includes("qeTemplate"), true);
  assert.equal(result.text.includes("RFID-RAW-P1.template.zpl"), false);
  assert.equal(result.text.includes("qeProfileKey"), true);
  assert.equal(result.text.includes("qePrinterIp"), true);
  assert.equal(result.text.includes("qePrinterPort"), true);
  ...
  assert.equal(result.text.includes("quick-edit-preview-panel"), true);
  assert.equal(result.text.includes("qePreviewSvgHost"), true);
  assert.equal(result.text.includes("qeSelectedObjectPanel"), true);
  assert.equal(result.text.includes("qeInspectorControls"), true);
  assert.equal(result.text.includes("qeNudgeControls"), true);
  assert.equal(result.text.includes("<option value=\"fit\">Fit</option>"), true);
  assert.equal(result.text.includes("Fit -25%"), true);
  assert.equal(result.text.includes("Fit -50%"), true);
  assert.equal(result.text.includes("Fit -75%"), true);
  assert.equal(result.text.includes("<option value=\"0.25\">25%</option>"), true);
  ...
  assert.equal(result.text.includes("Template Lab Product"), true);
  assert.equal(result.text.includes("Print Quick Edit Settings"), true);
  assert.equal(result.text.includes("Back to Full Template Lab"), true);
  assert.equal(result.text.includes("Open Print Health"), true);
  assert.equal(result.text.includes("Unsaved Quick Edit changes"), true);
  assert.equal(result.text.includes("Promote Dynamic Template to Production"), false);

  const templateLabScript = fs.readFileSync(path.join(REPO_ROOT, "public", "offline", "template-lab.js"), "utf8");
  const quickEditScript = fs.readFileSync(path.join(REPO_ROOT, "public", "offline", "template-quick-edit.js"), "utf8");
  ...
  assert.equal(quickEditScript.includes("QCSamplePounds-P3.template.zpl"), true);
  assert.equal(quickEditScript.includes("DEFAULT_PROFILES"), true);
  assert.equal(quickEditScript.includes("P8:FG"), true);
  assert.equal(quickEditScript.includes("P3:SAMPLE_POUNDS"), true);
  assert.equal(quickEditScript.includes("/api/print/template-lab/catalog"), true);
  assert.equal(quickEditScript.includes("/api/print/template-preview"), true);
  assert.equal(quickEditScript.includes("/api/print/template-test-send"), true);
  assert.equal(quickEditScript.includes("/api/print/template-lab/profile"), true);
  assert.equal(quickEditScript.includes("/api/print/template-lab/promote"), false);
  assert.equal(quickEditScript.includes("currentRenderSnapshot"), true);
  assert.equal(quickEditScript.includes("latestRenderSignature"), true);
  assert.equal(quickEditScript.includes("currentRenderSignature"), true);
  assert.equal(quickEditScript.includes("snapshotOrBlock"), true);
  assert.equal(quickEditScript.includes("renderSnapshot: snapshot"), true);
  assert.equal(quickEditScript.includes("renderedZplSha256"), true);
  assert.equal(quickEditScript.includes("dynamicTemplateSha256"), true);
  assert.equal(quickEditScript.includes("wirePreviewObjectHandlers"), true);
  assert.equal(quickEditScript.includes("selectPreviewObject"), true);
  assert.equal(quickEditScript.includes("data-qe-control"), true);
  assert.equal(quickEditScript.includes("setSelectedValue"), true);
  assert.equal(quickEditScript.includes("setDirty(\"Re-render after edit"), true);
  assert.equal(quickEditScript.includes("nudgeSelected"), true);
  assert.equal(quickEditScript.includes("ArrowLeft"), true);
  assert.equal(quickEditScript.includes("confirmProof.checked"), true);
  ...
  assert.equal(quickEditScript.includes("localStorage.setItem(QUICK_EDIT_ZOOM_STORAGE_KEY"), true);
  assert.equal(quickEditScript.includes("localStorage.getItem(QUICK_EDIT_ZOOM_STORAGE_KEY"), true);
  assert.equal(quickEditScript.includes("function calculatedFitScale"), true);
  assert.equal(quickEditScript.includes("if (mode === \"fit-25\") return 0.75;"), true);
  assert.equal(quickEditScript.includes("if (mode === \"fit-50\") return 0.50;"), true);
  assert.equal(quickEditScript.includes("if (mode === \"fit-75\") return 0.25;"), true);
  assert.equal(quickEditScript.includes("return fitScale * fitScaleMultiplier(mode);"), true);
  assert.equal(quickEditScript.includes("\"0.25\": \"25%\""), true);
  assert.equal(quickEditScript.includes("\"0.5\": \"50%\""), true);
  assert.equal(quickEditScript.includes("\"0.75\": \"75%\""), true);
  const quickEditApplyZoomMatch = quickEditScript.match(/function applyZoom\(\) \{([\s\S]*?)\n  \}/);
  assert.ok(quickEditApplyZoomMatch);
  assert.equal(quickEditApplyZoomMatch[1].includes("setDirty"), false);
  assert.equal(quickEditApplyZoomMatch[1].includes("renderedZpl"), false);
  assert.equal(quickEditApplyZoomMatch[1].includes("profileOverrides"), false);
  assert.equal(quickEditApplyZoomMatch[1].includes("postJson"), false);
  ...
  assert.equal(templateLabScript.includes("pinPreviewToggle"), true);
  assert.equal(templateLabScript.includes("showRenderDetailsToggle"), true);
  assert.equal(templateLabScript.includes("updateRenderDetailsVisibility"), true);
  assert.equal(templateLabScript.includes("renderDetailsShouldShow"), true);
  assert.equal(templateLabScript.includes("setCompactRenderState"), true);
  assert.equal(templateLabScript.includes("templateLabBody"), true);
  assert.equal(templateLabScript.includes("templateLabControlsScroll"), true);
  assert.equal(templateLabScript.includes("setPreviewPinState"), true);
  assert.equal(templateLabScript.includes("updateStickyPreviewOffset"), true);
  assert.equal(templateLabScript.includes("element.id === \"pinPreviewToggle\""), true);
  assert.equal(templateLabScript.includes("element.id === \"showRenderDetailsToggle\""), true);
  assert.equal(templateLabScript.includes("activeAreaFilters.has(\"field-fit-debug\")"), true);
  assert.equal(templateLabScript.includes("areas.includes(\"field-fit-debug\") && activeAreaFilters.has(\"field-fit\")"), false);
  assert.equal(offlineCss.includes("@media print"), true);
  assert.equal(offlineCss.includes(".template-lab-print-report"), true);
  assert.equal(offlineCss.includes(".quick-edit-print-report"), true);
  assert.equal(offlineCss.includes(".quick-edit-workbench"), true);
  assert.equal(offlineCss.includes(".quick-edit-preview-frame.zpl-preview-frame"), true);
  assert.equal(offlineCss.includes(".quick-edit-preview-frame.zoomed-out"), true);
  assert.equal(offlineCss.includes(".quick-edit-zoom-status"), true);
  assert.equal(offlineCss.includes(".quick-edit-inspector"), true);
  assert.equal(offlineCss.includes(".quick-edit-command-grid"), true);
  assert.equal(offlineCss.includes(".quick-edit-shell > :not(.quick-edit-print-report)"), true);
  ...
  assert.equal(offlineCss.includes(".compact-render-state-badge"), true);
  assert.equal(offlineCss.includes(".template-lab-preview-rail"), true);
  assert.equal(offlineCss.includes(".template-lab-body.preview-pinned .template-lab-preview-rail"), true);
  assert.equal(offlineCss.includes("position: fixed"), true);
  assert.equal(offlineCss.includes("max-height: calc(100vh - var(--template-lab-preview-top) - 14px)"), true);
  assert.match(offlineCss, /\.zpl-preview-frame[\s\S]*?overflow: hidden;/);
  assert.match(offlineCss, /\.zpl-preview-svg-host[\s\S]*?overflow: visible;/);
  assert.match(offlineCss, /\.template-lab-body\.preview-pinned \.template-lab-preview-rail[\s\S]*?overflow-y: auto;/);
  assert.equal(offlineCss.includes(".template-lab-body.preview-unpinned .template-lab-preview-rail"), true);
  assert.equal(offlineCss.includes(".template-lab-workbench,"), true);
