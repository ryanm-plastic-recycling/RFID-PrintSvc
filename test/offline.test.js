...
test("template lab page, preview, and test send stay outside production queue", async () => {
  ...
  assert.equal(result.text.includes("id=\"templateLabWorkbench\" class=\"template-lab-workbench preview-pinned\""), true);
  ...
  assert.equal(offlineCss.includes(".template-lab-workbench.preview-pinned .template-lab-preview-rail"), true);
  assert.equal(offlineCss.includes("position: sticky"), true);
  assert.equal(offlineCss.includes("max-height: calc(100vh - var(--template-lab-preview-top) - 14px)"), true);
  assert.equal(offlineCss.includes(".template-lab-workbench.preview-unpinned .template-lab-preview-rail"), true);
