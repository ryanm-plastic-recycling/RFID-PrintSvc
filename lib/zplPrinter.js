...
function fitZplBoxedText(value, options = {}) {
  ...
  const requestedFontW = Number.isInteger(options.fontWidth) && options.fontWidth > 0 ? options.fontWidth : selected.fontW;
  const requestedFontH = Number.isInteger(options.fontHeight) && options.fontHeight > 0 ? options.fontHeight : selected.fontH;
  const fontW = Math.max(minimum.fontW, Math.min(requestedFontW, maxFontWForBox));
  const fontH = Math.max(minimum.fontH, Math.min(requestedFontH, Math.round(fontW * 1.45)));
  ...
  return {
    ...
    fontW,
    boxW: boxWidth,
    boxH: Number.isInteger(options.boxHeight) && options.boxHeight > 0 ? options.boxHeight : null,
    maxChars,
    maxLines,
    alignment,
    borderThickness: Number.isInteger(options.borderThickness) ? options.borderThickness : null,
    truncated
  };
}
...
function buildFittedZplFieldValues(inputValues = {}, options = {}) {
  ...
  for (const [prefix, definition] of Object.entries(definitions)) {
    ...
    debug[prefix] = {
      ...
      boxW: fit.boxW,
      boxH: fit.boxH,
      borderThickness: fit.borderThickness,
